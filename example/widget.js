...
async function pageHistory(txId) {
  ...
  const result = await arweave.api.post('graphql', { query: historyQuery('vh-NTHVvlKZqRxc8LyyTNok65yQ55a_PJ1zWLb9G2JI', 'Test Page')});
...
// (1:0) <script>   import Modal from './components/modal.svelte'   import { pageHistory }
...
// (27:4) {#each hx as page}
function create_each_block(ctx) {
	let tr;
	let td0;
	let t0_value = /*page*/ ctx[4].timestamp + "";
	let t0;
	let t1;
	let td1;
	let t2_value = /*page*/ ctx[4].status + "";
	let t2;
	let t3;
	let td2;
	let a;
	...
	return {
		c() {
			...
			t5 = space();
			attr(a, "class", "link");
			attr(a, "href", a_href_value = "https://arweave.net/" + /*page*/ ctx[4].webpage);
...
// (13:0) <Modal open={historyDialog} on:click={() => historyDialog = false}>
function create_default_slot(ctx) {
	...
	handle_promise(pageHistory(), info);

	return {
		c() {
			h2 = element("h2");
			h2.textContent = "Permapage History";
			t1 = space();
			await_block_anchor = empty();
			info.block.c();
			attr(h2, "class", "text-xl mb-8");
		},
		m(target, anchor) {
...
function create_fragment(ctx) {
	...
	modal = new Modal({
			props: {
				open: /*historyDialog*/ ctx[0],
				$$slots: { default: [create_default_slot] },
				$$scope: { ctx }
			}
		});

	modal.$on("click", /*click_handler*/ ctx[2]);

	return {
		c() {
			button = element("button");
			button.textContent = "Show Page History";
			t1 = space();
			create_component(modal.$$.fragment);
			attr(button, "class", "btn btn-primary w-full");
		},
		m(target, anchor) {
			insert(target, button, anchor);
			insert(target, t1, anchor);
		...
		p(ctx, [dirty]) {
			const modal_changes = {};
			if (dirty & /*historyDialog*/ 1) modal_changes.open = /*historyDialog*/ ctx[0];
...
function instance($$self, $$props, $$invalidate) {
	...
	async function handleClick() {
		$$invalidate(0, historyDialog = true);
	}

	const click_handler = () => $$invalidate(0, historyDialog = false);
	return [historyDialog, handleClick, click_handler];
}
